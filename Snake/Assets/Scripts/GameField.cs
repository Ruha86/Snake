...
public class GameField : MonoBehaviour
{
    ...
    {
        // НОВОЕ: Создаём массив ячеек размерностью CellsInRow x CellsInRow
    ...
    {
        // НОВОЕ: Получаем ячейку по заданным координатам
        GameFieldCell cell = GetCell(x, y);

        // НОВОЕ: Если ячейка не была найдена, возвращаем (0, 0)
        if (cell == null)
        {
            return Vector2.zero;
        }
        // НОВОЕ: Возвращаем позицию найденной ячейки
